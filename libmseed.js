  ...
  <script>
    var Module = {
      locateFile: function (path) {
        if (path.endsWith('.wasm')) return 'libmseed.wasm';
        return path;
      },
...
<body>
  ...
  <script>
    ...
    document.getElementById("applyButton").addEventListener("click", function () {
      ...
      if (!window.wasmReady) {
        alert("WebAssembly not yet initialized. Please wait.");
        return;
      }
      ...
      Plotly.newPlot('waveform', [{ x: time, y: data, type: 'scatter', mode: 'lines', name: 'Waveform' }], {
        margin: { t: 30 }, xaxis: { title: 'Time (s)' }, yaxis: { title: 'Amplitude' }
      });
      ...
      Plotly.newPlot('fft', [{ x: freq, y: spectrum, type: 'scatter', mode: 'lines', name: 'FFT' }], {
        margin: { t: 30 }, xaxis: { title: 'Frequency (Hz)' }, yaxis: { title: 'Amplitude' }
      });
    });
  </script>
</body>
